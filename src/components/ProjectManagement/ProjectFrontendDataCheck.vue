<template>
          ...
          <div v-else v-for="(frontend, index) in frontendConfigs" :key="'frontend-' + index" class="configContainer">
            <div class="configBuildContainer">
              <div class="configHeader" @click="toggleConfig(index)">
                <span class="configTitle">{{ frontend.configName }}</span>
              </div>
              <div v-show="frontend.isExpanded" class="buildContainer">
                <b-container>
  ...
  const updateFrontendConfig = () => {
    if (props.selectedIndex !== -1) {
      frontendConfigs.value = props.allConfigs[props.selectedIndex]?.frontendConfigs.map(config => ({
        ...config,
  ...
  const toggleConfig = (index) => {
    frontendConfigs.value[index].isExpanded = !frontendConfigs.value[index].isExpanded;
  };
