<template>
          ...
          <div v-else  v-for="(backend, index) in backendConfigs" :key="'backend-' + index" class="configContainer">
            <div class="configBuildContainer">
              <div class="configHeader" @click="toggleConfig(index)">
                <span v-if="!backend.isEditing" class="configTitle">{{ backend.configName }}</span>
              </div>
              <div v-show="backend.isExpanded" class="buildContainer">
                <b-container>
  ...
  const updateBackendConfigs = () => {
    ...
    backendConfigs.value = props.allConfigs[props.selectedIndex]?.backendConfigs.map(config => ({
      ...config,
      isExpanded: false,
    })) || [];
  }
  ...
  backendConfigs.value[index].isExpanded = !backendConfigs.value[index].isExpanded;
};
